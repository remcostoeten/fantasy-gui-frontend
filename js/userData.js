...
$(document).ready(function () {
	...
	if (local.get('key') == '') {
		...
		console.log(
			'%c Logged in as ' + forums.username,
			'background: #444; color: #bada55; padding: 100px 300px; border-radius:2px; font-size: 40px',
		);
	}
	...
	if (forums.id < 1000) {
		$('#userAvatar').attr(
			'src',
			`https://fantasy.cat/forums/data/avatars/l/0/${forums.id}.jpg`,
		);
	} else {
		let f = forums.id.toString().charAt(0);
		$('#userAvatar').attr(
			'src',
			`https://fantasy.cat/forums/data/avatars/l/${f}/${forums.id}.jpg`,
		);
	}
